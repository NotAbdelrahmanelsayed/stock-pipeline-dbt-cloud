...
def transform_stock_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing_cols:
        logger.warning(
            f"Missing expected columns: {missing_cols}. Output may be malformed."
        )
...
def download_stock_data(
    ...
    try:
        if is_full_load:
            ...
            if not start_date:
                raise ValueError(
                    "start_date and end_date must be provided if is_full_load is False."
                )
            logger.info(
                f"Downloading data for {tickers} from {start_date} to {date.today()}"
            )
